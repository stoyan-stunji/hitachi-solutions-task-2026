...
- `Парсване на карта`: чете от конзолата; валидира дали броят редове и броят колони е в интервала `[2,100]`, дали има между `1` и `3` астронавта, дали има точно `1` дестинация; поддържа символите: `S1`, `S2`, `S3` (астонавти/начални точки), `F` (космическа станция/крайна точка), `0` и `O` (проходимо поле), `X` (непроходимо поле/астероид);
